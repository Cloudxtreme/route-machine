...
function MemoryMonitor(options) {
	...
	// 200MB
	options.gracefulWait = options.gracefulWait || 120;
	// 120 seconds
	options.checkInterval = options.checkInterval || 60;
	// 60 seconds
	options.logHandler = options.logger.create('router', 'MemoryMonitor', raft.config.get('log_session'));

	this.options = options;
	this._servers = [];
	this._closing = 0;
	this._exited = false;
	this.metrics = raft.metrics(cluster.worker.id + ':' + raft.config.get('router:metrics_session'), process.pid);
	this.stats = {
		startTime : Date.now(),
		requests : 0,
		connections : 0,
	...
	this.log = function(msg) {
		this.options.logHandler.log('(worker #' + cluster.worker.id + ') ' + msg);
	};
