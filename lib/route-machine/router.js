...
Router.prototype.start = function() {
	...
	if (cluster.isMaster) {
		...
		this.server.proxy.on('end', function(req, res) {
			...
			if (!req.droplet)
				return self.log.log(self.gen_log_info(req, res));
			...
			req.droplet.log.log(self.gen_log_info(req, res, req.droplet));
		});
...
Router.prototype.register_droplet = function(url, host, port, log_session, name, ps, app_id) {
	...
	console.log(url, host, port, log_session, name, ps, app_id)
...
Router.prototype.serveText = function(req, res, proxy, msg) {
	...
	res.end();
	self.log.log(self.gen_log_info(req, res));
};
